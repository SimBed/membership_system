...
class MalevolentBookingsTest < ActionDispatch::IntegrationTest
  ...
  test 'attempt by client to book class with provisonally expired package should fail' do
    ...
    assert_difference 'Wkclass.count', 3 do
      post admin_wkclasses_path, params: { wkclass: { workout_id: 3, start_time: '2022-03-19 10:30:00', instructor_id: 3, max_capacity: 6 } }
      post admin_wkclasses_path, params: { wkclass: { workout_id: 3, start_time: '2022-03-20 10:30:00', instructor_id: 3, max_capacity: 6 } }
      post admin_wkclasses_path, params: { wkclass: { workout_id: 3, start_time: '2022-03-21 10:30:00', instructor_id: 3, max_capacity: 6 } }
    end
    ...
    assert_equal flash[:warning], ["The maximum number of classes has already been booked.","Renew you Package if you wish to attend this class"]
  end
  ...
  test 'admin' do
    #  should be able to make amendment to booking for provisonally expired package when change wont improve package terms
    # create 3 new classes
    log_in_as @admin
    assert_difference 'Wkclass.count', 3 do
      post admin_wkclasses_path, params: { wkclass: { workout_id: 3, start_time: '2022-03-19 10:30:00', instructor_id: 3, max_capacity: 6 } }
    ...
    # admin corrects class incorrecly logged as 'cancelled early' to no-show
    assert_difference '@attendance.amendment_count', 0 do
      patch admin_attendance_path(@attendance), params: { attendance: { status: 'no show' } }
    end
    assert_redirected_to admin_wkclass_path @attendance.wkclass, {no_scroll: true}
    assert_equal flash[:warning], "The purchase has provisionally expired. This change may not be possible without first cancelling a booking"
