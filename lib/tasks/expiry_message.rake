...
task expiry_message_send: :environment do
  Purchase.package_started_not_fully_expired.reject(&:pt?).each do |p|
    ...
    if p.days_to_expiry == Setting.expiry_message_days.to_i && p.client.purchases.package.not_started.reject(&:pt?).empty?
